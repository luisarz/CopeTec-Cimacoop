                ...
                <div class="row">
                    ...
                    <div class="col-4">
                        <div class="form-check form-switch form-check-custom form-check-solid me-10" style="margin:4px">
                            <input class="check-permiso form-check-input h-40px w-60px" type="checkbox" value="{{$modulo->id_modulo}}" id="modulo{{$modulo->id_modulo}}"/>
                    ...
                    <div class="row" style="margin-top:30px">
                        ...
                        @foreach($modulos as $sub_modulo)
                            @if($modulo->id_modulo==$sub_modulo->id_padre)
                                <div class="col-4">
                                    <div class="form-check form-switch form-check-custom form-check-solid me-10" style="margin:4px">
                                        <input class="check-permiso form-check-input h-30px w-50px" type="checkbox" value="{{$sub_modulo->id_modulo}}" id="modulo{{$sub_modulo->id_modulo}}"/>
...
<script>
    ...
    $(document).ready(function(){
        $(".check-permiso").change(function(){
            showLoading()
            if($(this).is(":checked")){
                $.post("/allowAccess",{id_modulo:$(this).val(),id_rol:$("#id_rol").val(),isActive:true},function(){
                    hidenLoading();
                })
            }
            else{
                $.post("/allowAccess",{id_modulo:$(this).val(),id_rol:$("#id_rol").val(),isActive:false},function(){
                    hidenLoading();
                })
            }
        })
    ...
    function setAccess(id_rol){
        showLoading()
        $.post("/getAllowAccess",{id_rol:id_rol},function(data){
