...
@endsection
@section('content')
    <form action="/captaciones/depositosplazo/put" method="post" autocomplete="off">
        {!! csrf_field() !!}
        {{ method_field('PUT') }}
        <input type="hidden" name="periodo_en_dias" id="periodo_en_dias">
        <input type="hidden" name="id_deposito_plazo_fijo" id="id_deposito_plazo_fijo" value="{{ $deposito->id_deposito_plazo_fijo }}">
            ...
            <div class="card-body">
                ...
                <div class="form-group row mb-5">
                    <div class="form-floating col-lg-6">
                        <select name="id_asociado" id="id_asociado" class="form-select" data-control="select2"
                            ...
                            @foreach ($asociados as $asociado)
                                @if ($asociado->id_asociado == $deposito->id_asociado)
                                    <option value="{{ $asociado->id_asociado }}" selected>{{ $asociado->nombre }} ->
                    ...
                    <div class="form-floating col-lg-3">
                        <input type="number" step="1" min="6" value="{{ $deposito->numero_certificado }}"
                            required class="form-control text-info text-bold" placeholder="Numero Certificado"
                            name="numero_certificado" />
                        <label># Certificado:</label>
                    </div>
                    <div class="form-floating col-lg-3">
                        <input type="date" value="{{ $deposito->fecha_deposito }}" max="{{ date('Y-m-d') }}" required
                ...
                <div class="form-group row mb-5">
                    ...
                    <div class="form-floating col-lg-3">
                        <input type="number" step="1" value="{{ $deposito->numero_cheque }}" class="form-control"
                    ...
                    <div class="form-floating col-lg-6">
                        <input type="number" step="1" min="6" required class="form-control"
                            value="{{ $deposito->monto_deposito }}" placeholder="Monto a Depositar" id="monto_deposito"
                            name="monto_deposito" />
                    ...
                    <div class="form-floating col-lg-6">
                        <select name="id_cuenta_depositar" id="id_cuenta_depositar" class="form-select"
                            ...
                            @foreach ($cuentas as $cuenta)
                                @if ($cuenta->id_cuenta == $deposito->id_cuenta_depositar)
                                    <option value="{{ $cuenta->id_cuenta }}" selected>
                                        {{ $cuenta->numero_cuenta }} -> {{ $cuenta->descripcion_cuenta }} -> {{ $cuenta->nombre }}
                                    </option>
                                @else
                    ...
                    <div class="form-floating col-lg-6">
                        <select name="plazo_deposito" id="plazo_deposito" class="form-select" data-control="select2"
                            ...
                            @foreach ($plazos as $plazo)
                                @if ($plazo->id_plazo == $deposito->plazo_deposito)
                                    <option value="{{ $plazo->id_plazo }}" selected>
                                        {{ $plazo->descripcion }} ->{{ $plazo->meses }} Meses
                                    </option>
                                @else
                ...
                <div class="form-group row mb-5">
                    <div class="form-floating col-lg-4">
                        <select name="interes_deposito" id="interes_deposito" class="form-select" data-control="select2"
                            ...
                            @foreach ($tasas as $tasa)
                                @if ($tasa->id_tasa == $deposito->interes_deposito)
                                    <option value="{{ $tasa->id_tasa }}" selected>
                                        {{ $tasa->valor }}% -> {{ $tasa->descripcion }}
                                    </option>
                                @else
                                    <option value="{{ $tasa->id_tasa }}">
                                        {{ $tasa->valor }}% -> {{ $tasa->descripcion }}
                                    </option>
                                @endif
                            @endforeach
                    ...
                    <div class="form-floating col-lg-4">
                        <input type="date" min="{{ date('Y-m-d') }}" value="{{ $deposito->fecha_vencimiento }}" required class="form-control text-danger"
                ...
                <div class="form-group row mb-5">
                    <div class="form-floating col-lg-6">
                        <input type="number" name="interes_total" value="{{ $deposito->interes_total }}" id="interes_total" class="form-control" require
                            readonly>
                        <label>Interes Total:</label>
                    </div>
                    <div class="form-floating col-lg-6">
                        <input type="number" name="interes_mensual" value="{{ $deposito->interes_mensual }}" id="interes_mensual" class="form-control" require
                            readonly>
